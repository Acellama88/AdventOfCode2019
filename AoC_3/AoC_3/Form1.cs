...
namespace AoC_3
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int[] arraySize = max_distance(line1, line2);

            int maxX = arraySize[1] - arraySize[3] + 1;
            int maxY = arraySize[0] - arraySize[2] + 1;

            int origin_x = arraySize[3] * -1;
            int origin_y = arraySize[2] * -1;
            ...
            foreach (string step in line)
            {
                char dir = step[0];
                string distance = step.Substring(1);
            ...
            System.IO.StreamWriter file = new System.IO.StreamWriter(@"C:\temp\map.txt");
            ...
            foreach (int[] point in crosspoints)
            {
                int dist = Math.Abs(point[0] - origin_x) + Math.Abs(point[1] - origin_y);
                if (dist < minDist)
                    minDist = dist;
            }
            MessageBox.Show(minDist + "");
        }

        private int[] max_distance(string l1, string l2)
        {
            int x = 0, y = 0;
            int max_north = 0, max_east = 0, max_south = 0, max_west = 0;
            ...
            return new int[] {max_north, max_east, max_south, max_west};
        }
            ...
            foreach (string step in line)
            {
                ...
                switch (dir)
                {
                    case 'U':
                        for (int i = 1; i <= dist; curstep++, i++)
                            if(map[x, y + i] > 0)
                                crosspoints.Add(map[x, y+i] + curstep);
                        y += dist;
                        break;
                    case 'R':
                        for (int i = 1; i <= dist; curstep++, i++)
                            if(map[x + i, y] > 0)
                                crosspoints.Add(map[x + i, y] + curstep);
                        x += dist;
                        break;
                    case 'D':
                        for (int i = 1; i <= dist; curstep++, i++)
